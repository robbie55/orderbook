...
enum class OrderType : std::uint8_t { Limit = 0, Market = 1, Ioc = 2, Fok = 3 };
...
struct PriceBand {
    ...
    [[nodiscard]] constexpr std::size_t size() const noexcept {
        return static_cast<std::size_t>(ceil - floor) + 1U;
    }
    [[nodiscard]] constexpr bool contains(Price p) const noexcept {
        return p >= floor && p <= ceil;
    }
    [[nodiscard]] constexpr std::size_t index(Price p) const noexcept {
        return static_cast<std::size_t>(p - floor);
    }
    [[nodiscard]] constexpr Price price_at(std::size_t i) const noexcept {
        return floor + static_cast<Price>(i);
    }
};
